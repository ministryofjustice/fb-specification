...
const getSchemaPath = (name) => {
  ...
  const namePath = name.endsWith('.schema.json') ? name : `${dir}/${schemaName}.schema.json`
  return namePath
}
const rawSchemas = {}
...
const doLoad = name => {
  ...
  refSchema = refSchema.replace(/"\$ref":\s*("(http:\/\/gov.uk)*\/schema\/v1.0.0\/definition\/condition")/g, '"CONDITIONREF": $1')
